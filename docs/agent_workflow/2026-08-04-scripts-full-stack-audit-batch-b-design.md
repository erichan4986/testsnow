...
The historical nested `.claude/worktrees/report-redesign` checkout contains an
old valuation implementation but is not imported by this worktree; it is
excluded from both edits and current-runtime reference counts.
...
- Repository references: definition and docstring example only.
- Active report-duty owner: `TechnicalCollector` obtains optional fund-flow
  rows and `technical_skills._bridge_technical_fund_flow` projects them into
  report context. This is not claimed to be an API-compatible replacement for
  direct external callers of the old Eastmoney helper.
- Not exported from `reporter.__init__` and not documented in README.
...
- top-level active owners remain: `fetch_tencent_quote`,
  `compute_pillar_scores`, and `_deterministic_conclusion`;
- existing collector and technical-skill behavior tests continue to prove
  optional fund-flow collection and context bridging.
...
tests/utils/test_data_source_fallback_hygiene.py
```
...
- Exact AST definition spans plus the package export total 92 lines; surrounding
  separators may make the textual deletion slightly larger.
