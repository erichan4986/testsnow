...
def test_a_share_history_provider_uses_exchange_prefixed_statement_symbols(monkeypatch):
    ...
    fake_akshare = SimpleNamespace(
        ...
        stock_financial_abstract=lambda symbol: calls.append(("indicator", symbol)) or _Frame([{"指标": "毛利率"}]),
    )
...
def test_a_share_history_provider_keeps_amount_rows_when_optional_indicator_fails(monkeypatch):
    def fail_indicator(symbol):
        raise RuntimeError("optional indicator unavailable")

    fake_akshare = SimpleNamespace(
        stock_profit_sheet_by_report_em=lambda symbol: _Frame([{"a": 1}]),
        stock_cash_flow_sheet_by_report_em=lambda symbol: _Frame([{"b": 2}]),
        stock_financial_abstract=fail_indicator,
    )
    monkeypatch.setitem(sys.modules, "akshare", fake_akshare)

    assert data_fetcher.fetch_structured_financial_history_rows("688385") == {
        "profit": [{"a": 1}], "cashflow": [{"b": 2}], "indicator": [],
    }
